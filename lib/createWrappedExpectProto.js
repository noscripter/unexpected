        ...
        shift: function (subject, assertionIndex) {
            ...
            if (assertionIndex !== -1) {
                ...
                if (nextArgumentType.is('expect.it')) {
                    ...
                    return this.withError(function () {
                        return that.execute.apply(that.execute, [subject].concat(rest));
                    }, function (err) {
                        if (err.fixes) {
                            var shiftedFixes = {};
                            Object.keys(err.fixes).forEach(function (key) {
                                shiftedFixes[parseInt(key, 10) + 1 + that.offset + assertionIndex] = err.fixes[key];
                            });
                            err.fixes = shiftedFixes;
                        }
                        throw err;
                    });
                } else {
